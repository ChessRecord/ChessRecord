...
function countGames() {
  ...
  const { length: gameCount } = window.games;
  const tournamentCount = new Set(
    window.games.map(({ tournament }) => tournament || "Unknown"),
  ).size;

  els.gameCount.innerHTML = gameCount
    ? `${gameCount} ${gameCount === 1 ? "Game" : "Games"}`
    : "No Games";
  els.tournamentCount.innerHTML = tournamentCount
